...
const CoordProjectStatus = () => {
    ...
    setFilteredProjects(
        ...
        `${project.fullname}`.toLowerCase().includes(term) ||
    ...
    navigate(`/coordinator-view-project-progress/${projectID}`);
    }
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {currentProjects.length > 0 ? (
                    currentProjects.map((project, index) => (
                      <tr key={index}>
                        <td className="px-3 sm:px-6 py-4 whitespace-nowrap text-left">{`${project.fullname}`}</td>
